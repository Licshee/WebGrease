...
namespace WebGrease.Css.Visitor
{
    ...
    public sealed class FloatOptimizationVisitor : NodeTransformVisitor
    {
        ...
        public override AstNode VisitTermNode(TermNode termNode)
        {
                ...
                if (match.Success)
                {
                    ...
                    if (fullNumber == 0)
                    {
                        ...
                        if (string.IsNullOrEmpty(units) || units == "%" || LengthUnits.IsMatch(units))
                        {
                            ...
                            return new TermNode(termNode.UnaryOperator, "0", termNode.StringBasedValue, termNode.Hexcolor, termNode.FunctionNode, termNode.Comments);
                        }
                        else
                        {
                            // but we can't drop the units for Angles, Times, Frequencies, or Resolutions,
                            // and we shouldn't drop them for any unknown units.
                            return new TermNode(termNode.UnaryOperator, string.Concat("0", units), termNode.StringBasedValue, termNode.Hexcolor, termNode.FunctionNode, termNode.Comments);
                        }
                    }
                    ...
                    return new TermNode(termNode.UnaryOperator, string.Concat(leftNumber, rightNumber, units), termNode.StringBasedValue, termNode.Hexcolor, termNode.FunctionNode, termNode.Comments);
                }
            }
            ...
            return new TermNode(termNode.UnaryOperator, numberBasedValue, termNode.StringBasedValue, termNode.Hexcolor, funcNode, termNode.Comments);
        }
    }
}