...
namespace WebGrease.ImageAssemble
{
    ...
    internal static class ArgumentParser
    {
        ...
        /// </summary>
        private static IList<InputImage> inputImageList;
        ...
        /// </summary>
        internal static ReadOnlyCollection<InputImage> InputImageList
        {
            get
            {
        ...
        internal static void ParseArguments(string[] input, IList<InputImage> inputImages)
        {
        ...
        private static void ParseInputFilePaths(string filePaths)
        {
            ...
            var imageList = new List<InputImage>();
                ...
                if (!string.IsNullOrEmpty(path))
                {
                    if (path.Contains("|"))
                    {
                        ...
                        if (pairValue.Length == 2 && pairValue[0].Trim().Length > 0 && pairValue[1].Trim().Length > 0)
                        {
                            ...
                            var inputImage = new InputImage(pairValue[0].Trim());
                            ...
                            var result = imageList.Where(il => il.ImagePath.Equals(inputImage.ImagePath, StringComparison.OrdinalIgnoreCase) && il.Position == inputImage.Position);
                            if (result != null && result.Count() > 0)
                            {
                                // If Image already exists then throw exception for duplicate input files
                                throw new ImageAssembleException(string.Format(CultureInfo.CurrentCulture, ImageAssembleStrings.DuplicateInputFilePathsMessage, Paths, inputImage.ImagePath));
                            }
                            else
                            {
            ...
            var notExist = imageList.Where(il => !File.Exists(il.ImagePath)).ToList();
        ...
        private static List<InputImage> ConvertToInputImageList(string[] imagePaths)
        {
            if (imagePaths == null)
            {
            ...
            var inputImages = new List<InputImage>();

            foreach (var imagePath in imagePaths)
            {
                if (!string.IsNullOrEmpty(imagePath))
                {
                    var inputImage = new InputImage(imagePath);
