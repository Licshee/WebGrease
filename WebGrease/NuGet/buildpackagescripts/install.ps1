...
Function VSExecution($toolsPath, $package, $project)
{
    ...
    $relativePackageUri = GetRelativeUri $project.FullName $toolsPath
    $msbuild.Xml.AddProperty("WebGreaseBuildTools", $relativePackageUri.ToString().Replace([System.IO.Path]::AltDirectorySeparatorChar, [System.IO.Path]::DirectorySeparatorChar))
...
Function CommandLineExecution($toolsPath, $package, $project)
{
    ...
    $webGreaseBuildLocation = [System.Xml.Linq.XName]::Get("WebGreaseBuildTools", $defaultNameSpace.NamespaceName)
...
{
    CommandLineExecution $toolsPath $package $project
}
ELSE
{
    VSExecution $toolsPath $package $project
}
