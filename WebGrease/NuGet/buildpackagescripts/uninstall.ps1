...
Function VSExecution($toolsPath, $project)
{
	...
	# now remove our property that points to this package path, "WebGreaseBuildTools"
	foreach ($property in $msbuild.Properties)
	{
		if ($property.Name -eq "WebGreaseBuildTools")
		{
...
Function CommandLineExecution($toolsPath, $project)
{
	...
	$msnRfPackageElement = [System.Xml.XPath.Extensions]::XPathSelectElement($projXDoc.Root, "//my:WebGreaseBuildTools", $namespaceManager)
