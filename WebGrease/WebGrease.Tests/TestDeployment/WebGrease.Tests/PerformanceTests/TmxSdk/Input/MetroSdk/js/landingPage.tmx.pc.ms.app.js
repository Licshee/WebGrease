...
/* C:\Projects\Workspace2\MSNMetro\Main\MetroSDK\MetroSDK\Content\Source\js\Tmx\Ms\basePage.tmx.pc.ms.js */

/* WGINCLUDE: C:\PROJECTS\WORKSPACE2\MSNMETRO\MAIN\METROSDK\METROSDK\CONTENT\SOURCE\JS\TMX\MS\..\..\Shared\Behaviors\contentZoom.tmx.pc.ms.js */
/////////////////////////////////////////////////////////////////////////////////
//
// File: contentZoom.tmx.pc.ms.js
// Defines: contentZoom
// Dependencies: jQuery, viewAware, requestAnimationFrame, deviceGroup, dpi
// Description: This behavior helsp other behaviors manage the msContentZoom (pinch, zoom & -ms-viewport in ie10).
//
// Copyright (c) 2012 by Microsoft Corporation. All rights reserved.
//
/////////////////////////////////////////////////////////////////////////////////

require(["jquery", "viewAware", "requestAnimationFrame", "deviceGroup", "dpi", "mediator"], function ($, viewAware, requestAnimationFrame, deviceGroup, dpi, mediator)
{
    var $html = $("html");
    var currentViewport;
    var styleId = "ie10scale";
    var initialContentZoomFactor;
    var userZoomAllowed = true;
    var snapMin = 40;
    var viewportWidth = 60;
    var $currentStyleElement;
    var isFullscreen = false;

    mediator.sub("fullscreen", function(newFullscreen)
    {
    ...
    viewAware.listen(function (mode)
    {
        requestAnimationFrame(function ()
        {
            // Store whether use zoom is allowed (Only in fill/full)
            userZoomAllowed = !((mode & viewAware.modes.SNAP));
            initialContentZoomFactor = $html[0].msContentZoomFactor;
        });
    });

    // define the mediq query to use for scaling.
    var mq = "(min-width: " + snapMin + "em) and (max-width: " + viewportWidth + "em)";
    var msMatchMediaDpiScale = window.msMatchMedia(mq);

    // The function that updates the viewport media query.
    var updateFn = function ()
    {
        requestAnimationFrame(function()
        {
            updateViewportWidth(msMatchMediaDpiScale.matches ? (((viewportWidth * dpi.screen) + 0.5) | 0) : 0);
        });
    };

    // Run the method every time the media query listener changes.
    msMatchMediaDpiScale.addListener(updateFn);
    // Run it initially.
    updateFn();

    var msMatchmediaIsLandscape = window.msMatchMedia("(orientation: landscape)");
    var lastOrientation = msMatchmediaIsLandscape.matches;
    msMatchmediaIsLandscape.addListener(function ()
    {
        var currentOrientation = msMatchmediaIsLandscape.matches;
    ...
    // Updates the viewport size for scaled / zoomed scenario's in ie10.
    function updateViewportWidth(newViewportWidth)
    {
        var $newStyleElement;
        var newStyle = null;
        
        // Checking this value which returns true on desktop and false on mobile ie10.
        if (!navigator.msManipulationViewsEnabled)
        {
            // IE10 on Phone in Desktop mode
            newStyle = "@media (orientation: portrait) { @-ms-viewport{width:60empx } } @media (orientation: landscape) { @-ms-viewport{width:80empx } } ";
        }
        else if (newViewportWidth && currentViewport != newViewportWidth)
        {
            // IE10 Desktop with zoom different want 100%.
            currentViewport = newViewportWidth;
            newStyle = '@media ' + mq + ' { @-ms-viewport{width:' + newViewportWidth + 'em!important} }';
        }
        
        if (newStyle)
        {
            $newStyleElement = $('<style id="' + styleId + '">' + newStyle + '</style>')
                .appendTo($("head"));
            ...
            $currentStyleElement = $newStyleElement;
        }
    }
    ...
    function setZoom(zoom)
    {
        if (userZoomAllowed) {
            $html
                .css("msContentZooming", zoom)[0].msContentZoomFactor = initialContentZoomFactor;
        }
    }
});

/* WGINCLUDE: C:\PROJECTS\WORKSPACE2\MSNMETRO\MAIN\METROSDK\METROSDK\CONTENT\SOURCE\JS\TMX\MS\..\..\Shared\Behaviors\languageSocial.js */
/////////////////////////////////////////////////////////////////////////////////
//
// File: languageSocial.js
// Defines: languageSocial
// Dependencies: jQuery, jqfn, safeCss, mediator, requestAnimationFrame, window
// Description: This ugly implementation piece of javascript is to properly position the social langauge bar in snap view only.
//
// Copyright (c) 2013 by Microsoft Corporation. All rights reserved.
//
/////////////////////////////////////////////////////////////////////////////////
define("languageSocial", ["jquery", "jqfn", "safeCss", "mediator", "window"], function ($, jqfn, safeCss, mediator, window)
{
    function applyBehavior($languageSocial)
    {
        var safeCssGroup = safeCss.createGroup();
        var footerSafeCssGroup;
        var languageSocialSafeCssGroup;
        var $footer = $("#foot footer");
        var $window = $(window);
        var footerMarginTopOrigin;
        ...
        function fullscreenChanged(isFullscreen)
        {
            if (isFullscreen)
            {
                // some race condition causes infopanes to break. Making this reset call yield.
                setTimeout(reset, 0);
            }
            else
            {   
        ...
        // to ensure this happens after.
        function callReposition()
        {
            clearTimeout(repositionTimeout);
            reposition();
            repositionTimeout = setTimeout(reposition, 230);
        }

        // to reposition the footer and #langauge-social properly.
        function reposition()
        {
            // get current dimension.
            var footerMarginTop = parseInt($footer.css("margin-top")) || 0;
            var footerMarginBottom = parseInt($footer.css("margin-bottom")) || 0;
            var footerHeight = $footer.height();
            var languageSocialOuterHeight = $languageSocial.outerHeight(true);
            var languageSocialBottom = parseInt($languageSocial.css("bottom")) || 0;

            if (footerMarginTop != (languageSocialOuterHeight + footerMarginTopOrigin) || languageSocialBottom != (footerHeight + footerMarginBottom))
            {
                footerSafeCssGroup.css("margin-top", (footerMarginTopOrigin + languageSocialOuterHeight) + "px");
                languageSocialSafeCssGroup.css("bottom", footerHeight + footerMarginBottom);
            }
        }
        ...
        {
            clearTimeout(repositionTimeout);
            footerSafeCssGroup.reset();
            languageSocialSafeCssGroup.reset();
        }
        ...
        return {
            setup: function ()
            {
                if ($footer.length)
                {
                    footerSafeCssGroup = safeCssGroup($footer);
                    languageSocialSafeCssGroup = safeCssGroup($languageSocial);
                    footerMarginTopOrigin = parseInt($footer.css("margin-top")) || 0;
                    callReposition();
                    mediator.sub("fullscreen", fullscreenChanged);
                    $window.on("resize", callReposition);
                }
            },
            ...
            teardown: function ()
            {
                if ($footer.length)
                {
                    ...
                    $window.off("resize", callReposition);
                    reset();
                }
            }
        };
...
require(["jquery", "viewState", "c.dom"], function ($, viewState)
{
    // Display the ad-choices image-link only if Javascript is enabled
    $(".ad a.adchoices").removeClass('adchoices').addClass('adchoicesjs');

    viewState.bind("languageSocial", "#language-social").mode(viewState.modes.SNAP);
});
